/*
 * This source file was generated by the Gradle 'init' task
 */
package ex04_oop;

import java.util.ArrayList;
import java.util.List;

public class App {

    public static void main(String[] args) {
        // 학생 1번 정보
        String stdName1 = "Kim";
        int stdAge1 = 20;
        int stdScore1 = 95;

        // 학생 2번 정보
        String stdName2 = "Lee";
        int stdAge2 = 21;
        int stdScore2 = 100;

        System.out.println(stdName1);

        // 객체지향 방식
        class Student {
            Student() { // 기본 생성자

            }
            // 생성자
            public Student(String name, int age, int score) {
                this.name = name;
                this.age = age;
                this.score = score;
            }
            // 멤버변수(필드)
            String name;
            int age;
            int score;

            // 멤버 메서드
            void introduce() {
                System.out.println("학생이름: " + name + " 학생 나이: " + age);
            }
        }
        // 객체지향은 그룹핑(단위 묶음)
        Student std1 = new Student();
        std1.name = "Kim";
        std1.age = 20;
        std1.score = 95;

        // Student std2 = new Student();
        // std2.name = "Lee";
        // std2.age = 21;
        // std2.score = 1000;

        // System.out.println(std1.name + ", " + std1.age);
        // System.out.println(std2.name + ", " + std2.age);

        Student std2 = new Student("Kim", 30, 85);
        std2.introduce();

        List<Student> students = new ArrayList<>();

        students.add(new Student("유고", 49, 95));
        students.add(new Student("요미", 30, 100));
        students.add(new Student("나현", 27, 80));

        
        Account myAcc = new Account();
        // myAcc.balance = 100000000;   // 캡슐화를 통해 잘못된 동작을 막아줌
        myAcc.deposit(2000000);
        
        System.out.println("계좌 잔고는: " + myAcc.getBalance());

        myAcc.withdraw(3000);   
        System.out.println("계좌 잔고는: " + myAcc.getBalance() + "입니다.");

        // ========================================================================================================================================================

        // 동물 클래스, 개 클래스 사용
        Animal ani = new Animal();
        ani.printName();
        ani.sound();

        Dog poppy = new Dog();
        poppy.printName();
        poppy.sound();

        // Animal poppyc = new Dog();
        // poppy.sound();  // 멍멍

        // Dog poppyd = new Animal();
        // poppy.sound();  // 오류

        // 업캐스팅
        Animal a1 = new Cat();
        a1.sound();

        // 다운캐스팅
        // 자식클래스로 다시 변환전에 정확한 타입을 확인
        if (a1 instanceof Cat) {
            Cat c1 = (Cat)a1;
            c1.printName();
        }

        System.out.println("복사 학습 =====> ");

        // 얕은 복사
        Student s1 = new Student();
        s1.name = "유고";

        Student s2 = s1;    // 복사
        s2.name = "이지";

        System.out.println(s1.name);
        System.out.println(s2.name);

        // 깊은 복사
        Student s3 = new Student();
        s3.name = "복이";
    
        Student s4 = new Student();
        s4.name = s3.name;

        s4.name = "나현";

        System.out.println(s3.name);
        System.out.println(s4.name);
    }
}


class Account { // 계좌 클래스
    private int balance;    // 잔고

    // 입금 
    public void deposit(int amount) {
        if (amount > 0) {
            this.balance += amount;
        }
    }

    // 출금
    public void withdraw(int amount) {
        if (amount > 0 && this.balance >= amount) {
            this.balance -= amount;
        }
    }

    // 잔고확인
    public int getBalance() {
        return this.balance;
    }
}

class Animal {
    String name = "동물";

    void sound() {
        System.out.println("동물소리");
    }

    void printName() {
        System.out.println(this.name);
    }
}

class Dog extends Animal {
    String name = "강아지";

    void sound() {
        System.out.println("멍멍");
    }

    void printName() {
        System.out.println(super.name + " / " + this.name);
    }
}

class Cat extends Animal implements iCage{
    String name = "고양이";

    void sound() {
        System.out.println("야옹!");
    }

    void printName() {
        System.out.println(super.name + " / " + this.name);
    }

    @Override
    public void checkIn() {
        System.out.println("집에 가둠!");
    }

    @Override
    public void checkOut() {
        System.out.println("집에서 쫓아냄. 청소할꺼임!");
    }
}

