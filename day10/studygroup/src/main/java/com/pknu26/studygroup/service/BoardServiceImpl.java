...
public class BoardServiceImpl implements BoardService {
    ...
    public void createBoard(BoardForm boardForm) {
        ...
        // #PRC10 - BoardForm에 있는 데이터를 가져와서 Board에 할당
        board.setTitle(boardForm.getTitle());
        board.setContent(boardForm.getContent());
        board.setWriter(boardForm.getWriter());
        // board.setCreatedAt(LocalDateTime.now());  // SYSDATE가 default이기 때문에 이 줄 생략가능
    ...
    public void updateBoard(BoardForm boardForm) {
        ...
        board.setContent(boardForm.getContent());
        board.setWriter(boardForm.getWriter());
        // board.setUpdatedAt(LocalDateTime.now());  // SYSDATE가 default이기 때문에 이 줄 생략가능
