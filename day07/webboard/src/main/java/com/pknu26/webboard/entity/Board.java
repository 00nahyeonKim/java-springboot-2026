...
public class Board {
    ...
    @Id  // Primary key
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "board_seq")  // bno를 1씩 자동 증가하도록 만듬
    ...
    @Column(length = 8000)  // Oracle에서는 varchar(4000) 이상 못씀
    ...
    // cascade : 부모테이블의 특정 데이터를 지우면, 자식테이블에 연결된 데이터도 모두 삭제
